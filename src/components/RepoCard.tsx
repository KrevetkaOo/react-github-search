...
export function RepoCard({ repo }: { repo: IRepo }) {
    ...
    return (
        <div className='border py-3 px-5 rounded mb-2 hover:shadow-md hover:bg-gray-100 transition-all cursor-pointer'>
            <h2 onClick={() => openLink(repo.html_url)} className='text-lg font-bold'>
                {repo.full_name}
            </h2>
            <p className='text-sm'>
            ...
            {!isFav ? (
                <button className='py-2 px-4 bg-yellow-400 mr-2 rounded hover:shadow-md transition-all' onClick={addToFavourite}>
                    Add
                </button>
            ) : (
